...
- **Untracking a Vault File Almost Deleted Two of Them:** `git rm --cached` leaves the file on disk, and that is the trap — the deletion comes one step later. In TL-08 the next `git checkout main` removed both working copies, and only git history brought them back. **Remove an ignored vault path on a branch made from the commit that tracks it, and merge that branch; never check out across it.** Back the file up outside the repository first. The vault paths `.gitignore` keeps out are `vault/.import/`, `vault/books/`, `vault/notes/`, `vault/preferences.json`, `vault/syntopicon/` and `vault/_ledger.json.corrupt-*`, the rescue copy of a ledger nobody could read (DS-18); `vault/_ledger.json` itself stays tracked, because only an import writes it. `tests/test_git_tracks_the_right_files.py` fails when an ignored vault path is not named here (TL-08, RD-08).
...
- **Modular File Rule:** a soft ceiling of about 300 lines a file. Past it, refactor out hooks, helpers or child components. Check 12 and CI hold every file to it.
