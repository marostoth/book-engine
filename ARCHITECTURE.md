...
- **The versions were too old, and the register's numbers were too kind:** the README asked for Node 18.0+, and the frontend test command uses `--experimental-strip-types`, which arrived in **Node 22.6.0**. It asked for Rust 1.75+; tauri 2.11.5 asks for 1.77.2, but the highest `rust-version` of the 493 crates in `Cargo.lock` is **1.88.0** (darling 0.24.1 and the icu crates), so 1.77.2 still cannot build it.
