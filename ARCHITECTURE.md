...
│       ├── audit-system.py          # Universal dynamic health orchestrator (12 vectors: Ledger, Anchors, Cards, Rust, TS, FTS5, GUI smoke test, Inspectional, Analytical, Syntopical, Elementary, Modularity & Isolation); the anchor and inspectional vectors use `is_heading`, so a paragraph that starts with a `#` is counted (IN-08)
│       ├── benchmark-fts.py         # Benchmarks SQLite FTS5 query latency (<15ms target); it reads the paragraphs of a chapter with `is_heading` (IN-08)
