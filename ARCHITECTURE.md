...
│       ├── audit-anchors.py         # Verifies paragraph anchor & footnote definition integrity; an import runs the same check before a book goes into the vault (IN-05); it reads every book of a folder of books, says how many chapters of how many books passed, and fails on a folder that holds no book (TL-04); it calls `allow_any_letter` first, as every command does (IN-09)
│       ├── audit-practice.py        # Audits zero-hallucination verbatim extractive study cards; every quiz option is chapter text and the right one comes right after the passage (LE-06); every cloze answer is a term that is text of the chapter, and its prompt shows the exact source with no marks and no second answer (LE-07); the answer and the exact source must be text of the paragraph the card names, and a run that finds no deck fails (TL-04); it calls `allow_any_letter` first, as every command does (IN-09)
│       ├── audit-system.py          # Universal dynamic health orchestrator (12 vectors: Ledger, Anchors, Cards, Rust, TS, FTS5, GUI smoke test, Inspectional, Analytical, Syntopical, Elementary, Modularity & Isolation); every vector fails when it read nothing, the GUI smoke test waits for `--launch` and refuses an app older than its Rust, and the modularity vector reads the backend and these scripts too (TL-04); the anchor and inspectional vectors use `is_heading`, so a paragraph that starts with a `#` is counted (IN-08); it prints a piece of a paragraph it finds fault with, so it calls `allow_any_letter` first (IN-09)
│       ├── benchmark-fts.py         # Benchmarks SQLite FTS5 query latency (<15ms target) on a copy of the index made with the SQLite backup API, never writing to the index the app reads, and it fails when a query finds no paragraph (TL-04); it reads the paragraphs of a chapter with `is_heading` (IN-08); it calls `allow_any_letter` first, as every command does (IN-09)
│       ├── process-inbox.py         # Automated fail-safe batch book intake pipeline & ledger manager; a book the vault already has stops unless --force; a file whose book id a book from another file has stops even with --force, and --book-id gives it its own id (IN-03); a book that fails the anchor and footnote check is `Failed`, stays out of the vault, and its file stays where it is (IN-05); the title of a book it wrote and the table at the end go through `say_what_was_done`, so a console that cannot take a letter never turns an import that worked into a run that failed (IN-09)
│       └── test-index-rebuild.py    # Verifies self-healing FTS5 index reconstruction from vault; it calls `allow_any_letter` first, as every command does (IN-09)
...
│       │   │   │   ├── DipStream.tsx            # Virtualized feed of chapter dip sampling cards; a paragraph that starts with a `#` is sampled like any other (IN-08)
...
│       │   │   ├── markdown.ts          # Chapter Markdown as a document of reader nodes: the blocks of the import, their anchors & the footnote texts (RD-03); `isHeadingBlock` is the reader's copy of the heading rule, and `bookText` reads a `&#35;` back as a `#` for a place that shows a chapter file itself (IN-08)
...
│           │   │   ├── chapter_blocks.rs    # Which blocks of a chapter file hold text of the book: a block is a heading only when Markdown reads it as one, so a paragraph that starts with a `#` and a word stays in search (IN-08)
│           │   │   ├── chapter_blocks_tests.rs # Heading rule tests: one to six marks and a space make a heading, a hash and a word does not, more than six marks does not, and search reads a `&#35;` back as a `#` (IN-08)
...
│           │   │   ├── indexer.rs           # Vault indexing, each book on its own (a file it cannot read is named; deleted books & removed chapters leave search) & FTS5 full-text search; a book is known by its folder name, and a renamed book folder is named; the rows hold the words of each paragraph, never its HTML (SEC-01); a chapter is read with `\n` line endings, so search finds the paragraphs that the reader shows (IN-06); only a heading is left out, so a paragraph that starts with a `#` can be found (IN-08)
│           │   │   ├── indexer_tests.rs     # Index tests: a broken book or chapter stops nothing, and deleted books & removed chapters leave search; search and the library know a book by its folder name; search finds each paragraph of a chapter with any line ending (IN-06); a hashtag, a `&#35;` and seven marks are all found, and a heading is not (IN-08)
...
│       │   ├── anchors.py               # Deterministic paragraph anchor (^p-xxx) injector; a chapter with 2 or 3 paragraphs gets head and tail samples that share no paragraph (IN-01); a block is a heading only when Markdown reads it as one, so a paragraph that starts with a `#` keeps its anchor (IN-08)
│       │   ├── assets.py                # Asset extraction, micro-asset filtering & page-level image suppression; a picture file is named by the 1-based page, so only the page that has a snapshot loses its pictures; two pictures of an EPUB that share a plain name both stay; a drawing the book holds as SVG loses its script (IN-10)
│       │   ├── batch.py                 # Batch document intake utility (.epub & .pdf); the title of a book it wrote goes through `say_what_was_done`, so a title the console cannot take is no failed book (IN-09)
...
│       │   ├── book_check.py            # The check of a book before it goes into the vault: every paragraph has its anchor, every footnote link has its note (IN-05), and no character is broken (CQ-02); it names a paragraph that starts with a `#` and has no anchor, where it used to pass it in silence (IN-08)
...
│       │   ├── cli.py                   # Command-line entrypoint (`book-ingest`); a book the vault already has is replaced only with --force; a --book-id that breaks the rule stops the import (SEC-03); --force alone never replaces a book from another file, and the stop gives the two ways on (IN-03); it calls `allow_any_letter` first and says what it wrote through `say_what_was_done`, so a Cyrillic title no longer makes it report a failure after it worked (IN-09)
│       │   ├── chapter_shape.py         # A page of a book that holds nothing but a title is no chapter: its headings join the chapter it introduces, which then carries its own name instead of the name of the division (CQ-04); `is_heading` is the one rule for what a heading is, for the import, the check, search and the audit (IN-08)
...
│       │   ├── console.py               # What a command prints: `allow_any_letter` lets a command print any letter of any book into a console, a pipe or a log file, and `say_what_was_done` prints what a command has already done without ever turning that work into a report of failure (IN-09)
│       │   ├── elementary.py            # Deterministic Flesch-Kincaid & reading time metrics
│       │   ├── endnotes.py              # Backmatter endnote relocation to inline footnotes; a note is one footnote line (IN-06); only a link to a note becomes a footnote, and any other link keeps its words (IN-02); `resolve_key` gives the document and the element a link names, and only when both are certain (IN-07)
│       │   ├── epub_parser.py           # XHTML chapter extractor & typography normalizer; book text that looks like HTML is written as text (SEC-01); a heading is one line, and the block where each element id starts is noted (CQ-01); every text a browser shows is written in any layout, with preformatted text as the book has it and a <br> as a line break, and no Project Gutenberg boilerplate (IN-02); a division and a chapter are known by the rules of `chapter_shape.py` (CQ-04); a `#` of the book text that would start a heading is written as `&#35;`, in a paragraph, a list item, a quote and a table row (IN-08)
...
│       │   ├── layout_stitcher.py       # Narrative sentence healing, layout reconciliation & callout hoisting; the two halves of a sentence that a page break, a photo credit or a figure label cut in two are joined again, and joining repeats until nothing more joins (CQ-03); no rule here names a book: the passage of one book that was written out sentence by sentence, and the names of four real photographers, are gone, and a credit is known by the agency that owns the picture (IN-10)
│       │   ├── ledger.py                # The intake ledger `vault/_ledger.json`: which file made which book. One module reads and writes it, and an import keeps the chapter and word counts of a book the ledger already knows (CQ-05)
...
│       │   ├── markdown_text.py         # Book text in chapter Markdown: a < or & that could be read as HTML is written as &lt; or &amp;, and read back (SEC-01); a `#` that would start a heading is written as `&#35;` and read back (IN-08)
...
│       │   ├── note_documents.py        # Which documents of an EPUB hold nothing but notes, and so make no chapter. A document is known by what it holds, never by the end of its file name (IN-07)
...
│       │   ├── pdf_sanitizer.py         # PDF slug normalization, drop-cap healing, heading & author sanitization; the slug of a name in another script gets a code (IN-03); the page's own marks come off the text and a small roman page number goes (`text_repair.py`, CQ-03); `book_language` reads the language a PDF says it is in, where every PDF used to be written down as English (IN-10)
│       │   ├── pipeline.py              # End-to-end ingestion pipeline coordinator; the book is built in its build folder, and goes into the vault only when it is whole and checked (IN-05); a page that holds no text joins the chapter after it, and no chapter is left with nothing to read (CQ-04); a document that holds nothing but notes makes no chapter, and the import says which one and why (IN-07); a book whose contents open nothing gets one entry for each chapter (IN-10)
│       │   ├── places.py                # A new import finds each old chapter and paragraph in the new text, and moves what the reader's files point to: the bookmark, reading time, notes, highlights and citations (IN-04); the moves go into the vault together with the new book (IN-05); a paragraph that starts with a `#` is one of the places a highlight can move to (IN-08)
...
│       │   ├── sample_generator.py      # Starter sample generator for development; it calls `allow_any_letter` first, as every command does (IN-09)
│       │   ├── scenarios.py             # Quiz cards that ask which sentence comes right after a passage: wrong options from other paragraphs of the chapter, no near-copies, the right option spread over A to D (LE-06); `tells_a_story` knows a story by its shape, a narrative phrase AND one person all through, where a list of phrases from one book called 13 paragraphs of the other books a story (IN-10)
...
│       │   ├── toc_links.py             # Links the EPUB contents to the import: each entry gets the chapter file that holds it & the paragraph where it starts (CQ-01); an entry that opens nothing at all goes, and an entry that only groups others stays (CQ-04); `contents_of_chapters` gives a book with no contents one entry for each chapter (IN-10)
...
│       │   ├── test_any_book.py         # Any-book tests: a story is known by its shape and no rule names a book, the picture of the page before a snapshot stays, two figures of a part keep two files, two pictures that share a plain name both stay, an SVG loses its script, a PDF says its own language, and a book with no contents gets one entry per chapter (IN-10)
...
│       │   ├── test_fresh_clone.py     # Fresh clone tests: one `npm install` reaches the frontend, one lock file holds its 274 packages, the README's Node and Rust versions match `engines` and `rust-version`, and the verification steps make a book before they check one (TL-06)
...
│       │   ├── test_one_check.py       # One-command tests: `npm run check` runs every tool and stops at the first failure, each tool's settings are at the root, every rule that is off says why, and the workflow runs the same three groups on every push (TL-05)
...
│       │   ├── test_audits_check_something.py # Audit honesty tests: every check fails when it read no book, no chapter, no deck or no ledger line; a cloze card must quote the paragraph it names; the launch check refuses an app older than its code; the benchmark never writes to the index it measures (TL-04)
...
### One Command Runs Every Check (TL-05)
- **What went wrong:** nothing ran automatically. There was no `.github/` folder at all, no ESLint, no Prettier, no rustfmt settings, no clippy settings, and no one command. Measured before this change: `cargo fmt --check` wanted 60 of the 70 Rust files, clippy had 6 warnings, ruff with a named rule set reported **1,012** problems and would reformat 55 of 88 files, and mypy, already set to `strict = true`, reported **41** errors. Every one of those is zero now.
- **The one command:** `npm run check` runs `check:rust`, then `check:web`, then `check:python`, joined with `&&` so the first failure stops the rest. Between them they run rustfmt, clippy with `-D warnings`, `cargo test`, `tsc`, ESLint, the frontend tests, `ruff check`, `ruff format --check`, mypy and pytest. `.github/workflows/check.yml` runs the same three on every push and every pull request, on Windows, because that is what the backend is built for.
- **Settings at the root:** ruff's lived in `packages/ingestion/pyproject.toml`, which ruff never reads for a file outside that folder, so the scripts of `.agent/skills` were checked at ruff's own 88 columns: 292 lines were called too long when only 39 of them are longer than the 120 this repository writes to. `ruff.toml` at the root reaches both. `rustfmt.toml` says `max_width = 120` for the same reason: at rustfmt's own 100 the reformat touched 65 files and added 3,382 lines; at 120 it touched 60 and added 1,947.
- **Rules are named, not inherited:** ruff's own list grows with every release, so `ruff.toml` names the thirteen groups it runs. Five rules are off, each with the reason written above it. The plainest: **RUF001** calls the curly apostrophe `’` ambiguous, and this repository reads books that write one — `ingest/cloze.py` matches it on purpose, so "fixing" it would break the reading of those books.
- **What the linters found that tests had not:** a real crash. `ReaderMoves._highlights` (`ingest/places.py`) asked for the new place of a highlight, and `Places.of` gives nothing when the new text has no such chapter; the next line read `.chapter_file` off that nothing. Mypy named it. Two functions also used one name for two different things — `block` in `toc_links.py` and `parts` in `places.py` — which is what hid the first one from the type checker.
- **A fix can break what it tidies:** `ruff check --fix` deleted a deliberate re-export at the foot of `ingest/salience.py`, and the whole suite stopped at the import. `__all__` now says those names belong to that module, so nothing takes them away again.
- **What is not fixed:** ESLint reports **51 warnings** and no errors. Fifty-one is the number to drive down, and **TL-11** owns that work: 30 places set state inside an effect, spread over 26 components, and the rest read or write a ref while a component is drawing. Changing 26 components in the same commit as a linter that was not there before would have changed how the reader runs with nothing to catch it, because all 241 frontend tests read the functions of `src/lib` and none of them drew a component. TL-03 has since given this app a DOM test runner and the first 10 component tests, so TL-11 now has somewhere to put the proof; the 26 components still need their own tests written before they are changed.

### The Frontend Has a DOM Test Runner (TL-03)
- **What went wrong:** the review found 9 frontend tests, all of the pacer maths. By the time this was fixed there were **241**, but every one of them called a plain function in `src/lib/`. Not one drew a component: **62 components and 7 hooks had no test at all**, and there was no runner that could give one a page to draw into.
- **The second fault was quieter:** `npm test` named all 30 test files on one line. A test file left off that line never ran, and nothing said so. A test that is not run is worse than no test, because it reads as cover that is not there.
- **The runner:** `npm test` is `vitest run`, and `vite.config.ts` finds every `src/**/*.test.{ts,tsx}` by pattern. The default place is `node`, because 30 of these files test plain functions and a DOM would only slow them down; a file that needs one says `// @vitest-environment jsdom` on its first line.
- **The move cost almost nothing:** every file already asserted with `node:assert/strict`, which Vitest runs unchanged, and no file used subtests. What changed was the runner's own API: `mock.timers` became `vi.useFakeTimers`, and `t.after` became `onTestFinished`.
- **The move found a real difference:** node's `mock.timers.enable({ apis: ["setTimeout"] })` faked `clearTimeout` with it; Vitest's `toFake` is literal. Three tests failed at once, all of them debounces that pair `setTimeout` with `clearTimeout` — preferences and the reading place. Naming both in `toFake` restored them. The tests caught it, which is the point of having them.
- **The first component tests:** `components/BookSelector.test.tsx` opens the book list popover, clicks "Rescan library", and proves one click runs one rescan, that a running rescan cannot be started again, and that the note carries `role="status"` so a screen reader says it (DS-13, SI-05). `components/OmniSearchModal.test.tsx` mounts the search window with book text holding `<img src=x onerror=...>` and asks the document itself whether an element was made. `lib/searchSnippet.test.ts` already rendered to a **string** with `renderToStaticMarkup`; a string is not a page, and only this test proves what a browser does with it (SEC-01).
- **What is still open:** 60 components and all 7 hooks have no test. TL-11 needs them before its 26 components can be changed.

### A Fresh Clone Starts With the README Steps (TL-06)
- **What went wrong:** `npm install` at the root installed **3 packages**, because the root was not a workspace and had no postinstall step. `apps/desktop` needs **274**, and nothing installed them. Measured on a folder holding only the files git keeps: `npm --prefix apps/desktop test` gave **157 passed and 19 failed**, every failure `ERR_MODULE_NOT_FOUND` for `@tauri-apps/api`, `@tiptap/core` or `react`. After the fix the same folder gives **241 passed, 0 failed**, and `npm run build` finishes.
- **One workspace, one lock file:** `apps/desktop` is a workspace of the root package now, so one `npm install` installs both and npm keeps a single `package-lock.json` at the root. `apps/desktop/package-lock.json` is gone; it was a second list that nothing kept in step.
- **The versions were too old, and the register's numbers were too kind:** the README asked for Node 18.0+, and the frontend test command uses `--experimental-strip-types`, which arrived in Node 22.6.0. That number went into the README first and was wrong: a flag arriving is not the same as the tests running, and 22.6.0 cannot read `let answer!: T`. The floor is **Node 22.19.0**, the version the tests are proven on (TL-05). It asked for Rust 1.75+; tauri 2.11.5 asks for 1.77.2, but the highest `rust-version` of the 493 crates in `Cargo.lock` is **1.88.0** (darling 0.24.1 and the icu crates), so 1.77.2 still cannot build it.
- **The tools say it too:** `engines.node` in the root `package.json` and `rust-version` in `apps/desktop/src-tauri/Cargo.toml` carry the same two numbers, so npm and cargo tell a reader themselves instead of failing somewhere inside a dependency. `tests/test_fresh_clone.py` fails when the README and those two fields drift apart.
- **A check needs something to check:** the verification steps read the vault, and a clone's vault is empty, so `audit-anchors.py` failed at step 3 (TL-04 made it say so instead of passing). The README now makes a book first with `python -m ingest.sample_generator --vault vault`, a command that already existed and was in no document. No book text goes into git.

### A Check That Read Nothing Is Not a Pass (TL-04)
- **What went wrong:** several checks reported success on empty or missing input. Measured on an empty vault: the ledger check said "0 books, 0 binaries synced" and passed, the anchor and asset check said "0 anchors, 0 assets (0 chs)" and passed, and the guardrail against made-up practice cards said "0 decks found" and passed. `audit-anchors.py` printed "All chapters passed" and stopped with 0 for a folder that does not exist.
- **The shortest way to run it checked nothing:** `audit-anchors.py` reads `vault/books` when nobody names a folder, and that folder holds book folders, not chapter files. It globbed `*.md` in the folder it was given and never went a level down, so the call this repository documents read no file at all. It now reads every book of a folder of books, says how many chapters of how many books passed, and fails on a folder with no book in it.
- **The same gate stands in front of the vault:** `book_problems` (`ingest/book_check.py`) is what an import runs before a book goes into the vault (IN-05). It read no file and reported nothing wrong for a folder with no chapter, so a book with no chapter would have passed. A folder with no chapter file is a problem now.
- **A card must quote the paragraph it names:** every cloze card carries an `Anchor:` line, and `audit-practice.py` searched the whole chapter for the answer and the exact source. A card could name one paragraph and take its words from another one, and the audit still called it grounded. It reads the cited paragraph now. `audit-practice.py` also stopped with 0 when it found no deck, which every script reads as a pass; it stops with 1.
- **The audit does not start the app on its own:** the launch check ran on every audit. It opens a window, reads the reader's own vault and the live search database, and it took the release binary whenever there was one. Measured: the release binary was **7 days older** than the newest Rust file, and the debug binary beside it was newer. It waits for `--launch` now, takes whichever binary was built last, and fails a binary older than the Rust it was built from instead of starting it.
- **A benchmark does not write to what it measures:** `benchmark-fts.py` opened the live search database of the app read-write, set its journal mode, made tables in it, and inserted a row per paragraph of the vault when it found the index empty. It copies the index with the SQLite backup API and measures the copy. It also prints what each query found, because the fastest query there is, is one that finds nothing, and it fails when a query finds nothing. Measured on the owner's index: **9 of the 10 queries match 9 or fewer of 10,292 paragraphs**.
- **A line that says the same thing whatever it found:** the modularity check printed "N source files <= 300 lines (0 DB leaks)" beside the word FAIL. It says how many files are over the line and how many databases leaked. It also read the Python of the import and the TypeScript of the app and nothing else, so the Rust of the backend and the scripts of `.agent/skills/` were exempt from the rule they enforce; adding them names 8 more files, `audit-system.py` itself the longest of them.
- **The rules told agents not to look:** `AGENTS.md` said "Do not manually inspect files; rely entirely on the orchestrator's output", which is how these bugs stayed hidden. It says to read the files a failed check names.
...
- **A story is known by its shape:** `tells_a_story` (`ingest/scenarios.py`) needs two things at once, a phrase only a story says **and** one person all through (3 "he" or "she" words in every 100). A list of phrases alone called 13 paragraphs of Principles of Marketing and The Wealth of Nations a story, where a campaign "picked up by the media" and wine "drank in Great Britain" are not stories; being about one person alone called 190 paragraphs of The Wealth of Nations a story. Six of the phrases named the trade and the tools of one person in one book and are gone.
- **A picture of a page stays with its page:** `pymupdf4llm` names a picture file by the **1-based** page, so page index 30 gives `-0031-`. `suppress_page_images` (`ingest/assets.py`) also took `-0030-`, the file of the page **before**, and deleted a picture of a page that had no snapshot at all: 4 pictures of Mind Over Markets and **48 of Principles of Marketing**.
- **Two figures keep two pictures:** `figure_file_name` (`ingest/vector_figures.py`) carries the whole number of the figure. The name used to carry the minor number only, so Figure 2.1 and Figure 3.1 of one part wrote one file and one figure showed the picture of the other: **3 figures of Principles of Marketing** did.
- **Two pictures that share a plain name both stay:** a book can hold `images/fig1.png` and `notes/fig1.png`. The second used to write over the first, so one picture was lost and the other was shown twice. The second now keeps a mark of its own place in the book.
- **A drawing that carries a script loses it:** an SVG is markup, not pixels. The reader shows every picture through an `<img>` tag, where no script of a picture runs, but the file lives in the vault, so `svg_without_script` takes the script out before the file is written (SEC-01 does the same for a `<` of the book text).
- **A book says what language it is in:** `book_language` (`ingest/pdf_sanitizer.py`) reads `/Lang` from the PDF catalog. Every PDF used to be written down as English. Neither of your two PDFs says anything, so both stay English, and a book in another language no longer says it is English.
- **A book always has contents:** `contents_of_chapters` (`ingest/toc_links.py`) gives a book whose contents open nothing one entry for each chapter, the way the PDF import already makes parts for a PDF with no outline (IN-01). The code said it would do this and wrote an empty list instead.

### A Command May Print Any Letter of Any Book (IN-09)
- **What went wrong:** importing a book whose title holds Cyrillic or Greek letters printed `[-] Ingestion failed`, a traceback and the exit code 1, after every file of the book was written. Windows gives a command that writes into a pipe or into a log file the code page of the machine, such as cp1252, and those letters are not in it, so printing the title raised `UnicodeEncodeError` and the last `except` of the command called it a failed import.
- **The inbox skill was worse:** the table at the end sat outside every `try`, so a title in the ledger stopped the whole run with a bare traceback, with the book in the vault, in the ledger, and its file already moved to `inbox/processed/`.
- **The first rule:** `allow_any_letter` (`ingest/console.py`) gives `sys.stdout` and `sys.stderr` UTF-8 and `errors="replace"`. Every command of the repository calls it first. A real Windows console already takes UTF-8 (measured), so this changes nothing there and cures the pipe and the log file. A stream that cannot or will not change its encoding is left as it is, and a letter that no stream can take is printed as a `?`, never as a crash.
- **The second rule:** `say_what_was_done` (`ingest/console.py`) prints lines about work that is already finished, and never lets printing them make the command fail. `ingest/cli.py`, `ingest/batch.py` and `process-inbox.py` say what they wrote through it, so neither a letter the console cannot take nor a pipe whose reader went away (`command | head -3`) can turn a book that is in the vault into a report of failure. Each sentence is made before any of them is printed, so a real fault in a command is never swallowed. When a line cannot be printed, one line of plain letters on the error stream says the work is done.
- **What still reports a failure:** everything that really failed. A file that is no book, a book id that breaks the rule, a book the vault already has, and a book that fails the anchor and footnote check all stop the command with 1, as before.
- **Your own books:** none of the four would have hit this. Every title, author, file name and chapter title of them is in cp1252. The Principles of Marketing text holds 11 characters that are not (9 minus signs, one `Ğ`, one `ē`), deep in the body, where only `audit-system.py` could ever print one.

### A Paragraph That Starts With a `#` Is Still a Paragraph (IN-08)
- **What went wrong:** every block that started with a `#` counted as a heading, in the import, the book check, search, the audit and the reader's dip stream. A heading gets no paragraph anchor, so such a paragraph could not be searched, cited or marked, and nothing said so.
- **Two faults, two answers.** `#1 rule of the market`, a hashtag and `####### seven marks` are text that Markdown never read as a heading: they only needed the right rule. `# PRICES OF WHEAT`, a paragraph of The Wealth of Nations, is one that Markdown does read as a heading, and the text alone cannot say which it is.
- **The one rule:** `is_heading` (`ingest/chapter_shape.py`) is true for one to six `#` marks and then a space, a tab or the end of the line. `db/chapter_blocks.rs` and `isHeadingBlock` (`src/lib/markdown.ts`) are the same rule for search and for the reader.
- **How the import writes such a `#`:** `escape_heading_mark` (`ingest/markdown_text.py`) writes the first mark as `&#35;`, the way a `<` of the book text is written as `&lt;` (SEC-01). `&#35; PRICES OF WHEAT` keeps its anchor, the reader shows it as the book has it, and `search_text` (Rust) reads the reference back as a `#`, so search finds the word behind it. Only a line Markdown would read as a heading is written that way; `#1 rule` keeps every character.
- **Every kind of block, not only a paragraph:** Markdown reads a heading inside a list item and inside a quote too, and a `#` that starts the second line of a table cuts the table in two. So the mark is written for a paragraph, a list item, a quote and a table row. Preformatted text is the one block that keeps every character, because `_preformatted_block` already writes its own marks as character references.
- **The words a card and a highlight read are clean:** `clean_preview_text` (`ingest/anchors.py`) is the one place that previews, practice cards and moved highlights read a block through, and it writes the `&#35;` back as a `#`, so no `35` of a reference can ever look like a word of the book.
- **What a book of yours gains:** The Wealth of Nations holds one such paragraph, `# PRICES OF WHEAT` in `ch-14.md`, the caption of the wheat price table. The next import of that book gives it an anchor; the paragraphs below it in that chapter move up one number, and `ingest/places.py` moves what your files point to, as it does for any import.
- **What this cannot fix:** a PDF is read into Markdown by `pymupdf4llm`, which writes its own headings, so the import cannot tell a heading of the PDF from a line of its text that starts with `# `. Your two PDF books hold no such line (measured on all 1,139 pages).

### A Document of Nothing but Notes Is No Chapter (IN-07)
- **What went wrong:** the import dropped every document whose file name ended in `notes`, `endnotes`, `footnotes` or `backmatter`. That rule is wrong both ways, and both ways were seen: a real chapter named `ch03-fieldnotes.xhtml`, and an afterword named `backmatter.xhtml`, were dropped with no word said; and a document that says `epub:type="endnotes"` but is named `rear.xhtml` became a chapter of notes read twice.
- **Why a document of notes is skipped at all:** a book often keeps its notes at the back. The import puts each of those notes at the foot of the chapter that cites it, so importing that document as a chapter as well would write the same words twice.
- **What decides it now:** `documents_of_only_notes` (`ingest/note_documents.py`). A document holds only notes when the book itself says so, or when taking out every note that another document already shows as a footnote leaves nothing but headings. The book says so through `epub:type` or `role` (EPUB 3 and DPUB-ARIA), and only on the body or on the one and only block inside it: a chapter may hold its own `<ol epub:type="endnotes">` and is still a chapter.
- **The answer never depends on the order of the spine:** the notes each document cites are read from the whole book before the first chapter is built, so a document of notes may sit anywhere, before or after the chapters that cite it.
- **Only a note another document cites counts:** a note that only its own page cites already goes to the foot of that page. `resolve_key` (`ingest/endnotes.py`) gives the document and the element a link names, and gives nothing when the document is not certain. `resolve_target` still finds such a note by its name alone, so the reader sees the footnote; the document that holds it is simply not counted as a document of notes.
- **Where the rule cannot be sure, the document is kept.** Losing a chapter is the fault this rule exists to stop, and a note read twice is the smaller harm. So a document of notes that nothing cites is a chapter, and its words stay: the old rule deleted them. A document that is part notes and part prose is a chapter too, and the notes the chapters cite are in it as well as at their feet.
- **The import says so:** a document that makes no chapter is named on the screen with the reason, where before it went in silence.
- **The owner's books do not change:** The Wealth of Nations has no document of notes at all (36 chapters and 387,386 words before and after, the same words in the same order), and the sample book's `notes.xhtml`, whose 4 notes its chapters all cite, makes no chapter as before. The import now says why.
...
- **Why the numbers went stale:** the inbox was the only thing that wrote them. A book imported again from the command line (`ingest/cli.py`) wrote a new `_meta.json` and left the ledger saying the numbers of the older import. Both PDF books of the owner's vault drifted that way: the Dalton book said 78,445 words against 78,374 in the book, and the Kotler book 455,709 against 456,426. The two EPUB books, imported once and never again, still agreed.
- **An import keeps the numbers true:** `keep_numbers_true` (`ingest/ledger.py`) writes the chapter and word counts of the book into every line of the ledger for that book. `ingest_epub` and `ingest_pdf` both call it, so the numbers follow the book whoever runs the import.
- **An import never adds a book to the ledger:** only the inbox takes a file in, because only the inbox knows the file it moved into `inbox/processed/`. A book imported from the command line that the ledger does not know stays unknown to it, and nothing is written. A vault with no ledger gets none.
- **One module, not two copies:** `ingest/ledger.py` reads and writes the file, in one step so a stopped run never leaves half of it. `.agent/skills/process-inbox.py` uses that module in place of its own reading and writing, and the ledger's path follows the vault.
- **The audit says when they disagree:** vector 1 of `.agent/skills/audit-system.py` compares each line with the book's own `_meta.json` and names both numbers. A line whose book is not in the vault is left out: its numbers are the record of an import that happened, and no book is there to check them against. So the Kotler line stays as it is until that book is imported again.
...
**A Book May Hold Any Letter (TL-10):** Two faults in `db/fsrs_parser.rs`, both from mixing characters and bytes. The option parser checked `rest.chars().nth(2) == Some(')')`, a count of **characters**, and then cut `rest[3..]`, a count of **bytes**: `(①) Twice a month.` panics, because `①` is three bytes long and the cut lands inside it. Rust stops the whole program there, and that panic rolled back the entire deck sync, so one option in a book that numbers with circled digits, or in any non-Latin script, lost every card of the run. `rest.len() > 3` was the same mistake in the guard. The citation parser took an offset from `l.strip_prefix("<!--").and_then(|s| s.find("citation:"))`, which searches the text **after** the `<!--`, and used it on the whole line: four bytes out, so `<!-- citation: ch-01.md#^p-003 -->` named the chapter `tion: ch-01.md` and the card pointed at a file in no vault — not a crash, a quietly wrong answer. Both use `split_once` and `char_indices` now, which cut where they found and can never land inside a letter. `string_slice` is a warning in `Cargo.toml`, so `npm run check` fails on a new cut. The review counted 19 such cuts and there were **32** by the time it was fixed; there are **21** now, in `search_text.rs`, `highlights.rs`, `notes.rs` and `indexer.rs`, and every one is deliberate: each of those seven functions carries `#[expect(clippy::string_slice, reason = "...")]` saying why its position is always a letter boundary, and `expect` rather than `allow` so clippy asks for the attribute back once the cut is gone. `indexing_slicing`, which the finding also named, is **not** on: 97 sites, most of them in tests, and indexing a `Vec` you just built is a different hazard from cutting UTF-8. The six style warnings the review counted are already 0, fixed by TL-05's `-D warnings`.

**The Cache Is Set Up Once Per File, Not Once Per Call (SI-04):** Sixteen places call `open_or_create_db`, and every one of them used to run the whole setup again: thirteen statements of `CREATE TABLE IF NOT EXISTS`, a `PRAGMA table_info` migration check, and a look at whether the dictionary needed seeding. Each statement says `IF NOT EXISTS`, so it changed nothing and nobody noticed. Measured on the reader's real index, 10,292 paragraphs, on a copy taken with the SQLite backup API: a bare open is **0.19 ms** and an open with the setup is **2.86 ms**, so the setup cost **2.67 ms of every call**, including every search. `market*` was 15.9 ms and is 12.3 ms on a connection that is already set up, which is the difference between missing the documented 15 ms and meeting it. The setup now runs on the first open of a file in this process and not again, remembered by path in `PREPARED` (`db/schema.rs`), because a test build gives every `Sandbox` its own cache. `PRAGMA user_version` is what says the tables are still there, so a file that was deleted and opened again is built from nothing once more: two tests of DS-01 delete it on purpose. There is no shared connection and no lock around one, so a search during a startup index run is still served side by side by WAL. `synchronous` and `temp_store` live in the connection rather than the file and are set on every open; `journal_mode = WAL` lives in the file and is set with the tables. `db/open_cost_tests.rs` counts the setups per thread, because the saving is time and a test cannot hold a time still.

**The Speed in the Documents Is the Speed That Is Measured (SI-04):** `README.md` promised "sub-millisecond search" and the other documents "sub-15ms", and `benchmark-fts.py` agreed, because its ten queries matched between **1 and 338** paragraphs of 10,292 and took 0.08 to 1.10 ms between them. The search a reader actually waits for is a two-letter prefix, which `MIN_SEARCH_CHARACTERS` allows (SI-03): `th*` matches **87%** of the index and takes **54 ms**. TL-04 made this benchmark fail when a query found nothing; it did not stop a query that finds almost nothing. It now measures two kinds of search and holds each query to its own limit, never the average of the lot: a real word under **15 ms**, and the broadest search the app allows under **120 ms**. A broad query must reach at least 10% of the index, or the run fails and says it is measuring the easy case only. `tests/test_audits_check_something.py` fails when the documents name a limit the benchmark does not hold, when a broad query is no shorter than a word, and when one slow query among fast ones would be averaged away.

**A Hash You Write Down Is a Promise (SI-06):** `index_chapter` saves a hash of every chapter it wrote and skips that chapter next time when the text still gives the same answer. That hash came from `std::collections::hash_map::DefaultHasher` through a function called `md5_hash`, which was not MD5. `DefaultHasher` promises nothing about its output: the standard library may change it in any release, and the day it did, every chapter of every book would be read and written again, in silence, and the first run after a Rust update would simply be slow for no reason anyone could name. It is `sha256_of` now (`sha2`, already in `Cargo.lock` because tauri's own build uses it), and `db/content_hash_tests.rs` pins it to the SHA-256 of "abc" published in FIPS 180-4, so a change to the algorithm cannot pass with the tests still green. The repository already knew this rule: `db/card_identity.rs` writes its own FNV-1a for card ids, with a comment saying why `DefaultHasher` will not do and a test pinning the ids. Nothing checked that the rule was kept everywhere, and the index was the place it was not. `tests/test_stable_hashes.py` checks it now, for the hasher and for the name. Changing the algorithm makes every saved hash stale, so the first run after this change reads every chapter once, the same one-time cost `SEARCH_ROWS_FORM` exists for.

**Nothing Watches the Vault (SI-05):** There is no file system watcher, and the `notify` crate is in neither `Cargo.toml` nor `Cargo.lock`. `index_vault` runs twice: when the window opens (`useBookSession.ts`) and when the reader clicks "Rescan library" at the bottom of the book list (`lib/libraryRescan.ts`, DS-13). So a book imported or edited while the app is open shows after one click, not by itself. This diagram, `README.md` and `AGENTS.md` all described a watcher that was never written, and the same line of `AGENTS.md` named `sqlx`, which this backend has never used either. A watcher is not free here: the app writes into the vault all the time itself, notes, highlights, `bookmark.json`, `reading.jsonl` and the practice deck, so it would have to tell its own writes from the reader's or it would index in a loop. `tests/test_what_the_docs_promise.py` fails when a document promises a watcher again.
