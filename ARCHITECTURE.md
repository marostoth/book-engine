...
│       │   ├── pipeline.py              # End-to-end ingestion pipeline coordinator; the book is built in its build folder, and goes into the vault only when it is whole and checked (IN-05); a page that holds no text joins the chapter after it, and no chapter is left with nothing to read (CQ-04); a document that holds nothing but notes makes no chapter, and the import says which one and why (IN-07)
