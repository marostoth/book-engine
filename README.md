...
- **Node.js**: `22.19.0+` and `npm`. Vitest runs the frontend tests and asks for Node `^22.12.0`; jsdom, which
  gives those tests a page to draw into, asks for `^22.13.0`. The floor was `22.6.0` before Vitest, because the
  tests were TypeScript files that Node ran itself with `--experimental-strip-types`, and that flag arrived in
  22.6.0 — but a flag arriving is not the same as the tests running: 22.6.0 stopped at the `!` of `let answer!: T`
  with a `SyntaxError`. 22.19.0 is the version the tests are proven on. The root `package.json` says the same in
  `engines`, so `npm install` warns you when your Node is older, and `.github/workflows/check.yml` installs that
  exact version.
