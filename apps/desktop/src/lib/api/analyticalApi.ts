import { AnalyticalStore } from "../types/analytical";
...
export async function getAnalyticalData(bookId: string): Promise<AnalyticalStore> {
  if (isTauri) {
    try {
      return await tauriInvoke<AnalyticalStore>("get_analytical_data", { bookId });
    } catch (e) {
      console.warn("Tauri get_analytical_data failed, falling back to storage:", e);
    }
  }
...
export async function saveAnalyticalData(bookId: string, data: AnalyticalStore): Promise<void> {
  if (isTauri) {
    try {
      await tauriInvoke<void>("save_analytical_data", { bookId, data });
      return;
    } catch (e) {
      console.warn("Tauri save_analytical_data failed, caching in storage:", e);
    }
  }
