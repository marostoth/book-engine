...
/** The files that save a citation, a topic citation or a vocabulary word, or that open a chapter at a paragraph. */
const SAVING_FILES = [
  "../App.tsx",
...
test("no file that saves a citation or a word holds a made-up anchor (RD-04)", () => {
  for (const file of SAVING_FILES) {
    ...
    const madeUp = lines
      ...
      .filter((line) => !line.startsWith("//") && !line.startsWith("*") && !line.includes("placeholder"))
      .filter((line) => line.includes("^p-001"));

    assert.deepEqual(madeUp, [], `${file} writes an anchor of its own`);
  }
});
