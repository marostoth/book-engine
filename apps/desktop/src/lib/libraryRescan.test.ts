...
/** A backend whose vault has `library`, and whose index reads `chapters`. It records its calls in order. */
function fakeBackend(library: BookMetadata[] | Error, chapters: number | Error) {
  const calls: string[] = [];
  const backend: LibraryBackend = {
    ...
    indexVault: async () => {
      ...
      return { chapters_indexed: chapters, paragraphs_indexed: chapters * 40 };
    },
  };
...
test("the new book shows before search is done", async () => {
  let finishIndex: (summary: { chapters_indexed: number; paragraphs_indexed: number }) => void = () => {};
  const backend: LibraryBackend = {
  ...
  finishIndex({ chapters_indexed: 5, paragraphs_indexed: 200 });
