...
- An import builds the book in `vault/.import/<book-id>/`, checks every anchor and footnote there, and puts the book, its practice deck and the reader's moved files in the vault all together; a failed or stopped import changes nothing, and no file of an older import stays (IN-05)
- Two imports of the same file write the same bytes: `_meta.json` holds no time of the import, and `requirements.lock` gives the exact version of every package; install it with `python -m pip install -r requirements.lock` before `python -m pip install --no-deps -e .` (IN-05)
